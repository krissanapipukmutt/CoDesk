...
export class SupabaseRepo {
  ...
  async init() {
    if (!hasSupabaseEnv || !this.client) return false;
    try {
  ...
  async getProfile() {
    if (!this.client) return null;
    const { data, error } = await this.client.schema('codesk').rpc('current_profile');
  ...
  private sb() {
    if (!this.client) throw new Error('UNAUTHORIZED');
    return this.client.schema('codesk');
  }

  async listOffices(): Promise<Office[]> {
    const { data, error } = await this.sb().from('offices').select('*');
  ...
  async listDepartments(): Promise<Department[]> {
    const { data, error } = await this.sb().from('departments').select('*');
    if (error) throw error;
    return data.map((d) => ({
      id: d.id,
  ...
  async listSeats(): Promise<Seat[]> {
    const { data, error } = await this.sb().from('seats').select('*');
    if (error) throw error;
    return data.map((s) => ({
      id: s.id,
  ...
  async listEmployees(): Promise<Employee[]> {
    const { data, error } = await this.sb().from('employees').select('*');
    if (error) throw error;
    return data.map((e) => ({
      id: e.id,
  ...
  async listHolidays(): Promise<Holiday[]> {
    const { data, error } = await this.sb().from('holidays').select('*');
    if (error) throw error;
    return data.map((h) => ({
      id: h.id,
  ...
  async cancelBooking(bookingId: string, reason?: string | null): Promise<CancelBookingResult> {
    const { data, error } = await this.sb().rpc('cancel_booking', {
  ...
  async deleteDepartment(id: string) {
    const { error } = await this.sb().from('departments').delete().eq('id', id);
    if (error) throw error;
  }
  ...
  async deleteEmployee(id: string) {
    const { error } = await this.sb().from('employees').delete().eq('id', id);
    if (error) throw error;
  }
  ...
  async deleteHoliday(id: string) {
    const { error } = await this.sb().from('holidays').delete().eq('id', id);
    if (error) throw error;
  }

  async reportBookingsPerDay() {
    const { data, error } = await this.sb().from('v_bookings_per_day').select('*');
    if (error) throw error;
    return data;
  }

  async reportUtilization() {
    const { data, error } = await this.sb().from('v_utilization_by_department').select('*');
    if (error) throw error;
    return data;
  }

  async reportPopularSeats() {
    const { data, error } = await this.sb().from('v_popular_seats').select('*');
    if (error) throw error;
    return data;
  }

  async reportStatusSummary() {
    const { data, error } = await this.sb().from('v_booking_status_summary').select('*');
    if (error) throw error;
    return data;
  }

  async reportPeakTimes() {
    const { data, error } = await this.sb().from('v_peak_times').select('*');
    if (error) throw error;
    return data;
  }
